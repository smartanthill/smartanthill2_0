...
class TransmitterService(SAMultiService):
    ...
    def startService(self):
        try:
            ...
            self.log.error(
                exception.NetworkTransmitterConnectionFailure(self.options))
        ...
        self._litemq.consume(
            exchange="network",
            queue=self.name,
            routing_key="commstack->transmitter",
    ...
    def _make_link(self):
        ...
        if protocol_type not in ("serial",):
            raise exception.NetworkTransmitterUnsupportedProtocol(
                protocol_type)
    ...
    def in_packet_callback(self, packet):
        ...
        self._litemq.produce(
            "network", "transmitter->commstack", packet, dict(binary=True))
